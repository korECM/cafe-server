...
interface ReviewRepository : JpaRepository<Review, Long>, ReviewRepositoryCustom {
    @Query("select r from Review r left join fetch r._foodInfos left join fetch r._images left join fetch r._cafeKeywords join fetch r.footprint where r.id = :reviewId")
