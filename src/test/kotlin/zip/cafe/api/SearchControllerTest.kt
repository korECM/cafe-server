...
import zip.cafe.api.dto.CafeSearchRequestWithBoundary
...
class SearchControllerTest : WebMvcTestAdapter() {
    init {
        ...
        "좌표 기반 카페 이름 검색" {
            ...
            val minCafeId = 1L
            val limit: Long = 30
            val request = CafeSearchRequestWithBoundary(
                name = cafeName,
            ...
            every { searchService.searchCafeInBoundary(cafeName, visitPurposeList, foodList, keywordIdList, boundary, minCafeId, limit) } returns listOf(
            ...
            val response = mockMvc.post("/search/cafe-boundary") {
                contentType = APPLICATION_JSON
                content = objectMapper.writeValueAsString(request)
            }
        ...
        "카페 이름 검색" {
            ...
            every { searchService.searchCafe(cafeName, visitPurposeList, foodList, keywordIdList, minCafeId, limit) } returns listOf(
                createCafe(name = "멋진 북카페", totalScore = 13.0, reviewCount = 3, footPrintCount = 5),
                createCafe(name = "별로인 북카페", totalScore = 17.0, reviewCount = 5, footPrintCount = 8)
            )
